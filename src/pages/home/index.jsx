...
const HOME_COPY = {
    ...
    'zh-CN': {
        ...
        motion: {
            title: '数据集成，持续流动。',
            lead: '六种机制，覆盖并行负载、持续变更与可恢复故障中的数据流动。',
            ...
            mechanisms: [
                {diagram: 'parallel', title: '并行批读取', description: '拆分工作，让独立分片并行读取。', detail: '独立读取器处理已分配的源端分片。', note: 'JDBC 示例 · 分片分配取决于具体 source。'},
                {diagram: 'cdc', title: '快照 + CDC', description: '先完成快照，再持续捕获变更。', detail: '先读取已有行，再流式处理持续变更。', note: 'MySQL-CDC 示例 · startup.mode = initial。'},
                {diagram: 'transform', title: '行级转换', description: '记录落地前完成整形、过滤与补充。', detail: '在 pipeline 中应用行表达式和过滤条件。', note: 'SQL 示例 · 表达式 + 行过滤。'},
                {diagram: 'routing', title: '多表路由', description: '让每条记录沿着目标表路径流动。', detail: '表标识决定匹配的目标端写入器。', note: '逻辑路由 · 物理连接可能不同。'},
                {diagram: 'schema', title: 'Schema 演进', description: '新数据下游流动前先应用受支持的表结构变更。', detail: '沿兼容路径传播新增列。', note: 'Zeta 示例 · MySQL-CDC → JDBC MySQL。'},
                {diagram: 'checkpoint', title: 'Checkpoint 恢复', description: '故障后从已完成的 checkpoint 继续处理。', detail: '通过 checkpoint 屏障捕获状态。', note: 'Exactly-once 需要兼容的 source 与 sink 支持。'},
            ],
        },
        connectors: {
