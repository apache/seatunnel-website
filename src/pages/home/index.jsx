...
const HOME_COPY = {
    ...
    'zh-CN': {
        ...
        motion: {
            eyebrow: '动态演示',
            title: '六项机制，一个数据集成层',
            lead: '通过独立示例了解并行读取、CDC、行级转换、多表路由、Schema 演进与 Checkpoint 恢复。',
            openButton: '打开交互演示',
            frameTitle: 'SeaTunnel 数据集成机制交互演示',
            controls: {
                label: '动画演示控制',
                play: '播放动画',
                pause: '暂停动画',
            ...
            mechanisms: [
                {title: '并行读取', description: '将独立的数据源 Split 分配给多个 Reader。'},
                {title: '快照 + CDC', description: '先完成快照读取，再持续捕获变更。'},
                {title: '行级转换', description: '对单条记录执行表达式与过滤条件。'},
                {title: '多表路由', description: '根据表标识将数据路由至对应目标。'},
                {title: 'Schema 演进', description: '在数据向下游流动前应用已支持的 Schema 变更。'},
                {title: 'Checkpoint 恢复', description: '发生失败后从已完成的 Checkpoint 状态继续。'},
            ],
        },
        connectors: {
