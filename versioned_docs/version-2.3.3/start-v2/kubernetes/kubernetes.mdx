...
Then run the following commands to build the image:
```bash
docker build -t seatunnel:2.3.0-spark-3.3.3 -f Dockerfile .
```
Image `seatunnel:2.3.0-spark-3.3.3` need to be present in the host (minikube) so that the deployment can take place.

Load image to minikube via:
```bash
minikube image load seatunnel:2.3.0-spark-3.3.3
```
...
--set webhook.create=false  --set serviceAccounts.spark.name=spark
```
...
spec:
  type: Java
  mode: cluster
  image: seatunnel:2.3.0-spark-3.3.3
