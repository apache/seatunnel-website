...
```
Image `seatunnel:2.3.4-flink-1.13` need to be present in the host (minikube) so that the deployment can take place.
...
```
Image `seatunnel:2.3.4` need to be present in the host (minikube) so that the deployment can take place.
